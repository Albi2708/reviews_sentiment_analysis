...
* **Amazon Reviews 2023** (``raw_review_Electronics``) — streamed and
  stratified to a fixed 3-class sample; stars map 1-2 -> negative, 3 ->
  neutral, 4-5 -> positive.
* **Phenomenon set** — ``tests/fixtures/phenomenon_reviews.csv``, with an
  extra per-category breakdown.

Per dataset, writes ``metrics.json``, ``confusion_matrix.csv``,
``confusion_matrix.png``, and ``report.txt`` under ``--out-dir`` (default
``results/``). Calls ``analyze`` directly, so ``db.py`` logging is untouched.
Run with ``make eval`` or ``python evaluate.py``.
"""
...
def sample_amazon(
    ...
    Streams (no full download) until each class has ``2 * per_class``
    candidates, then shuffles with ``seed`` and keeps ``per_class`` per class.
    ``max_stream`` caps how far we read when a class is rare.
    """
...
def load_phenomenon(path: Path) -> list[dict[str, str]]:
    """Read the phenomenon CSV into ``{text, expected_label, category}`` dicts."""
...
def run_predictions(rows: list[dict[str, str]]) -> list[str]:
    """Call ``analyze`` on each row's text and return the resolved labels."""
    total = len(rows)
    preds: list[str] = []
...
def _per_class_report(y_true: list[str], y_pred: list[str]) -> dict[str, dict[str, float]]:
    """``classification_report`` as a json-serializable dict."""
...
def _save_confusion_heatmap(cm: list[list[int]], out_path: Path) -> None:
    """Save the confusion matrix as an annotated heatmap PNG (matplotlib lazy-imported)."""
    import matplotlib
    matplotlib.use("Agg")
...
def write_results(
    ...
    """Write metrics.json, confusion_matrix.{csv,png}, and report.txt under ``out_dir``."""
...
def evaluate_amazon(
    ...
    """Evaluate the Amazon stratified sample and write results to ``out_dir/amazon``."""
...
def evaluate_phenomenon(out_dir: Path, csv_path: Path) -> None:
    """Evaluate the phenomenon set (with per-category breakdown) to ``out_dir/phenomenon``."""
...
def main() -> None:
    """Parse CLI flags and run the requested dataset evaluation(s)."""
